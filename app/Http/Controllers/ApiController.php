...
class ApiController extends Controller
{
    ...
    public function GoogleApi($text,$url,$mainUrl,$from_language,$to_language){
        ...
        $LastData = [];
        $StringCorrections = [];
        $ProjectStringCorrectionsDoNot = [];

        $ProjectStringCorrectionsDoNot = ProjectStringCorrections::where('p_id',$ProjectDetails['id'])->where('type',1)->where('to_language',$to_language)->where('status',0)->get();
        foreach($ProjectStringCorrectionsDoNot as $ProjectString){
            $StringCorrections[] = StringCorrections::where('id',$ProjectString['string_correction_id'])->first();
        }
        if(sizeof($StringCorrections)>0){
            foreach($StringCorrections as $string){
                $search_arr[]  =  $string['do_not_translate_string'];
                $replace_arr[] = '<cite class="notranslate">' . $string['do_not_translate_string'] . "</cite>";

                $ExplodeData = explode(" ",$string['do_not_translate_string']);
                $length = sizeof($ExplodeData);
                $FirstData[] = $ExplodeData[0];
                $LastData[] = $ExplodeData[$length-1];
            }
        ...
        // $ProjectStringCorrectionsAlways = ProjectStringCorrections::where('p_id',$ProjectDetails['id'])->where('type',2)->where('to_language',$to_language)->where('status',0)->get();
        // foreach($ProjectStringCorrectionsAlways as $ProjectString){
        //     $StringAlways[] = StringCorrections::where('id',$ProjectString['string_correction_id'])->first();
        // }
        ...
        //     foreach($StringAlways as $string){
        //         $search_arr_Always[]  =  $string['do_not_translate_string'];
        //         $mainReplace_arr[] = "<para>" . $string['always_translate_as_string'] . "</para>";
        //     }
        // }


        /* End String Correction */
        ...
        if(empty($HtmlCodeDetails) || $HtmlCodeDetails == null){
            ...
            if($ProjectDetails['metadata_translation'] == 1){
                ...
                if(count($FullMetaData)>0){
                    ...
                    foreach($FullMetaData as $Mdata){
                        ...
                        $OriginalData = strip_tags($Mdata);
                        $i=0;
                        for($i=0; $i<count($FirstData); $i++){
                            if (strpos($OriginalData, $FirstData[$i]) !== false && strpos($OriginalData, $LastData[$i]) !== false){
                                $Mdata = str_replace($FirstData[$i], '<cite class="notranslate">'.$FirstData[$i], $Mdata);
                                $Mdata = str_replace($LastData[$i], $LastData[$i].'</cite>', $Mdata);
                            }
                        }

                        $output = $this->GoogleTranslateApi($sourceLanguage['sortname'], $destinationLanguage['sortname'], $Mdata);

                        $output = str_replace('<cite class="notranslate">', "", $output);
                        $output = str_replace('</cite>', "", $output);
                    ...
                    if(count($OriginalMetaDataArray)!=0){
                        ...
                        for($i=0; $i<count($OriginalMetaDataArray); $i++ ){
                            ...
                            $ParagraphDetails = ProjectData::where('p_id',$ProjectDetails['id'])->where('language_id',$to_language)->where('original_data', '=', $OriginalMetaDataArray[$i])->first();
                            ...
                            if($ParagraphDetails == null){
                                ...
                                $this->InsertProofReadingAssociative($ProjectDetails['id'], $ParagraphId, $SourceCode['id'], $to_language);

                                $this->InsertDataSection($ProjectDetails['id'], $ParagraphId, $SourceCode['id'], $to_language, 4);
                            }
                            elseif($ParagraphDetails != null){

                                $ProofreadingAssociative = ProofreadingAssociative::where('p_id',$ProjectDetails['id'])->where('paragraph_id',$ParagraphDetails['id'])->where('source_code_id',$SourceCode['id'])->where('language_id', $to_language)->first();
                                if($ProofreadingAssociative  ==  null){

                                    $this->InsertDataVersion($ParagraphDetails['id'], implode($TranslateMetaDataArray[$i]), 'M', 1);

                                    $this->InsertProofReadingAssociative($ProjectDetails['id'], $ParagraphDetails['id'], $SourceCode['id'], $to_language);

                                    $this->InsertDataSection($ProjectDetails['id'], $ParagraphDetails['id'], $SourceCode['id'], $to_language, 4);
                                }
                ...
                if($HtmlText !=null  || !empty($HtmlText) || $HtmlText != " " || $HtmlText != "\t" ){
                    ...
                    if(preg_match('/[A-Za-z]/', $TestHtmlText) || preg_match('/[0-9]/', $TestHtmlText)){
                        ...
                        if($TestHtmlText !== 'BESbswy' && str_word_count($TestHtmlText)>1){
                            ...
                            $ParagraphDetails = ProjectData::where('p_id',$ProjectDetails['id'])->where('language_id',$to_language)->where('original_data', '=', $DBMatchHtmlText)->first();
                            
                            $OriginalData = strip_tags($HtmlText);
                            $i=0;
                            for($i=0; $i<count($FirstData); $i++){
                                if (strpos($OriginalData, $FirstData[$i]) !== false && strpos($OriginalData, $LastData[$i]) !== false){
                                    $HtmlText = str_replace($FirstData[$i], '<cite class="notranslate">'.$FirstData[$i], $HtmlText);
                                    $HtmlText = str_replace($LastData[$i], $LastData[$i].'</cite>', $HtmlText);
                                }
                            }

                            $outputText = $this->GoogleTranslateApi($sourceLanguage['sortname'], $destinationLanguage['sortname'], $HtmlText);

                            $outputText = str_replace('<cite class="notranslate">', "", $outputText);
                            $outputText = str_replace('</cite>', "", $outputText);
                            ...
                            if($ParagraphDetails == null){
                                ...
                                $this->InsertDataSection($ProjectDetails['id'], $ParagraphId, $SourceCode['id'], $to_language, 5);
                            ...
                            elseif($ParagraphDetails != null){
                                ...
                                if($ProofreadingAssociative == null){
                                    ...
                                    $this->InsertDataVersion($ParagraphDetails['id'], $outputText, 'M', 1);

                                    $this->InsertProofReadingAssociative($ProjectDetails['id'], $ParagraphDetails['id'], $SourceCode['id'], $to_language);    
                                    
                                    $this->InsertDataSection($ProjectDetails['id'], $ParagraphDetails['id'], $SourceCode['id'], $to_language, 5);
                                }
                            }
                        }
                    }
            ...
            $TranslatedData = [];
            /* fecth paragraph ids */
            $ProofreadingAssociativeDetails = ProofreadingAssociative::where('p_id',$ProjectDetails['id'])->where('source_code_id',$SourceCode['id'])->where('language_id',$to_language)->get();
            foreach($ProofreadingAssociativeDetails as $Associative){

                $TranslatedData[] = ProjectData::where('id',$Associative['paragraph_id'])->where('p_id',$ProjectDetails['id'])->where('language_id',$to_language)->first();
            ...
            if($ProjectDetails['metadata_translation'] == 1){
                ...
                if(count($FullMetaData)>0){
                    ...
                    foreach($FullMetaData as $Mdata){
                        ...
                        for($i=0; $i<count($FirstData); $i++){
                            if (strpos($OriginalData, $FirstData[$i]) !== false && strpos($OriginalData, $LastData[$i]) !== false){
                                $Mdata = str_replace($FirstData[$i], '<cite>'.$FirstData[$i], $Mdata);
                                $Mdata = str_replace($LastData[$i], $LastData[$i].'</cite>', $Mdata);

                                $FirstPosition = strpos($Mdata,'<cite>');
                                $LastPosition = strpos($Mdata,'</cite>');

                                $SubStr = substr($Mdata,$FirstPosition,$LastPosition);

                                $Mdata = str_replace($SubStr, 'ce32f2f323', $Mdata);
                            }
                        }

                        $result = $this->AmazonTranslateApi($sourceLanguage['sortname'], $destinationLanguage['sortname'], $Mdata);

                        $result = str_replace('ce32f2f323', $SubStr, $result);

                        $result = str_replace('<cite>', "", $result);
                        $result = str_replace('</cite>', "", $result);
                ...
                if($HtmlText !=null  || !empty($HtmlText) || $HtmlText != " " || $HtmlText != "\t" ){
                    ...
                    if(preg_match('/[A-Za-z]/', $TestHtmlText) || preg_match('/[0-9]/', $TestHtmlText)){
                        ...
                        if($TestHtmlText !== 'BESbswy' && str_word_count($TestHtmlText)>1){
                            ...
                            for($i=0; $i<count($FirstData); $i++){
                                if (strpos($OriginalData, $FirstData[$i]) !== false && strpos($OriginalData, $LastData[$i]) !== false){
                                    $HtmlText = str_replace($FirstData[$i], '<cite>'.$FirstData[$i], $HtmlText);
                                    $HtmlText = str_replace($LastData[$i], $LastData[$i].'</cite>', $HtmlText);

                                    $FirstPosition = strpos($HtmlText,'<cite>');
                                    $LastPosition = strpos($HtmlText,'</cite>');

                                    $SubStr = substr($HtmlText,$FirstPosition,$LastPosition);

                                    $HtmlText = str_replace($SubStr, 'ce32f2f323', $HtmlText);
                                }
                            }

                            $outputText = $this->AmazonTranslateApi($sourceLanguage['sortname'], $destinationLanguage['sortname'], $HtmlText);

                            $outputText = str_replace('ce32f2f323', $SubStr, $outputText);
    ...
    public function DeeplTranslateApi($sourceLanguage, $destinationLanguage, $text){
        ...
        $request = $client->post('https://api.deepl.com/v2/translate?source_lang='.$sourceLanguage.'&target_lang='.$destinationLanguage.'&tag_handling=xml&ignore_tags=para&auth_key=dfec9916-d46f-c906-37be-c4f68ab97357',$options);
        $response=$request->getBody();
        $res=json_decode($response);
    ...
    public function InsertProjectData($id, $S_id, $original_data, $content_original_data , $translated_data, $content_translated_data, $data_section, $to_language, $status){
        ...
        $ProjectData->p_id = $id;
        $ProjectData->source_code_id = null;//$S_id;
        ...
        $ProjectData->data_section = null;//$data_section;
    ...
    /* For Data Section */
    public function InsertDataSection($ProjectId, $ParagraphId, $SourceCodeId, $language_id, $type){

        $DataSection = new DataSection();
        $DataSection->p_id = $ProjectId;
        $DataSection->paragraph_id = $ParagraphId;
        $DataSection->source_code_id = $SourceCodeId;
        $DataSection->language_id = $language_id;
        $DataSection->data_section = $type;
        $DataSection->save();

    }

    /* For ProofReading Associative */
    public function InsertProofReadingAssociative($ProjectId, $ParagraphId, $SourceCodeId, $language_id){

        $ProofreadingAssociative = new ProofreadingAssociative();
        $ProofreadingAssociative->p_id = $ProjectId;
        $ProofreadingAssociative->paragraph_id = $ParagraphId;
        $ProofreadingAssociative->source_code_id = $SourceCodeId;
        $ProofreadingAssociative->language_id = $language_id;
        $ProofreadingAssociative->save();
            ...
            if($ProjectDetails['metadata_translation'] == 1){
                ...
                if(count($FullMetaData)>0){
                    foreach($FullMetaData as $Mdata){
                        ...
                        for($i=0; $i<count($FirstData); $i++){
                            if (strpos($OriginalData, $FirstData[$i]) !== false && strpos($OriginalData, $LastData[$i]) !== false){
                                $Mdata = str_replace($FirstData[$i], '<para>'.$FirstData[$i], $Mdata);
                                $Mdata = str_replace($LastData[$i], $LastData[$i].'</para>', $Mdata);
                            }
                        }

                        // $i=0;
                        // for($i=0; $i<count($search_arr_Always); $i++){

                        //     preg_match_all('~<([^/][^>]*?)>~' ,  $Mdata, $TagDetails); 

                        //     preg_match_all('~<([</][^>]*?)>~' ,  $Mdata, $TagDetailsEnd); 

                        //     if (strpos($OriginalData, $search_arr_Always[$i]) !== false ){
                        //         $Mdata = str_replace($search_arr_Always[$i], $mainReplace_arr[$i], $Mdata);
                        //     }
                        // }
                        ...
                        for($i=0; $i<count($OriginalMetaDataArray); $i++ ){
                            ...
                            elseif($ParagraphDetails != null){
                                ...
                                if($ProofreadingAssociative  ==  null){
                                    ...
                                    $this->InsertDataVersion($ParagraphDetails['paragraph_id'], implode($TranslateMetaDataArray[$i]), 'M', 1);

                                    $this->InsertProofReadingAssociative($ProjectDetails['id'], $ParagraphDetails['paragraph_id'], $SourceCode['id'], $to_language);

                                    $this->InsertDataSection($ProjectDetails['id'], $ParagraphDetails['paragraph_id'], $SourceCode['id'], $to_language, 4);
                                }
                ...
                if($HtmlText !=null  || !empty($HtmlText) || $HtmlText != " " || $HtmlText != "\t" ){
                    ...
                    if(preg_match('/[A-Za-z]/', $TestHtmlText) || preg_match('/[0-9]/', $TestHtmlText)){
                        ...
                        if($TestHtmlText !== 'BESbswy' && str_word_count($TestHtmlText)>1){
                            ...
                            for($i=0; $i<count($FirstData); $i++){
                                if (strpos($OriginalData, $FirstData[$i]) !== false && strpos($OriginalData, $LastData[$i]) !== false){
                                    $HtmlText = str_replace($FirstData[$i], '<para>'.$FirstData[$i], $HtmlText);
                                    $HtmlText = str_replace($LastData[$i], $LastData[$i].'</para>', $HtmlText);
                                }
                            }
                            ...
                            //     // preg_match_all('~<([^/][^>]*?)>~' ,  $HtmlText, $TagDetails); 

                            //     // preg_match_all('~<([</][^>]*?)>~' ,  $HtmlText, $TagDetailsEnd); 

                            //     if (strpos($OriginalData, $search_arr_Always[$i]) !== false ){

                            //         $HtmlText = str_replace($search_arr_Always[$i], $mainReplace_arr[$i], $HtmlText);
                            //     }
                            // }

                            $outputText = $this->DeeplTranslateApi($sourceLanguage['sortname'], $destinationLanguage['sortname'], $HtmlText);

                            $outputText = str_replace('<para>', '', $outputText);
                            $outputText = str_replace('</para>', '', $outputText);
